...
export default class Edit_Question extends Component {
    ...
    async getQuestion() {
        const token = localStorage.getItem("token");
        var response = await axios.get(`http://localhost:3000/quizquestions/getQuestion/${this.props.match.params.id}`, {
            headers: {
                'auth-token': token
            }
        })
    ...
    async updateQuestion() {
        ...
        if (question && answer1 && answer2 && answer3 && answer4 && correctAnswer && id) {
            ...
            //     this.props.history.push("/users/login");
            // } else {
                var response = await axios.put(`http://localhost:3000/quizquestions/updateQuestion/${quizId}/${id}`, { question, answer1, answer2, answer3, answer4, correctAnswer }, {
                    headers: {
                        'auth-token': token
                    }
                });
