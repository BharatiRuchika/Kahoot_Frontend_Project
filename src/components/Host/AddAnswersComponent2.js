...
class Questions extends Component {
    ...
    async getQuestions() {
       ...
       var response = await axios.get(`http://localhost:3000/quizquestions/getQuestions/${this.props.quizToEdit._id}`,{
           headers:{
               'auth-token':token
           }
       })
    ...
    async deleteQuestion(id) {
        ...
        if(decoded.exp*1000<=Date.now()){
            ...
            var response =  await axios.delete(`http://localhost:3000/quizquestions/deleteQuestion/${id}`,{
           headers:{
               'auth-token':token
           }
       })
    ...
    async updateQuiz() {
        ...
        if (newName && newInfo) {
            ...
            if(decoded.exp*1000<=Date.now()){
                ...
                await axios.put(`http://localhost:3000/quiz/updateQuiz/${id}`, { quizName: newName, quizDescription: newInfo },{
                    headers:{
                        'auth-token':token
                    }
                })
    ...
    async handleUpdatedQuiz(id) {
        ...
        //     this.props.history.push("/users/login");
        // }else{
        var response = await axios.get(`http://localhost:3000/quiz/getQuiz/${this.props.quizToEdit._id}`,{
            headers:{
                'auth-token':token
            }
        })
