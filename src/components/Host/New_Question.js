...
class New_Question extends Component {
  ...
  let { question, answer1, answer2, answer3, answer4, correctAnswer } = this.state;
        ...
        console.log("id",id);
        if (question && answer1 && answer2 && answer3 && answer4 && correctAnswer) {
        var response =  await axios.post("https://my-kahoot-backend.herokuapp.com/quizquestions/addQuestions", { question, answer1, answer2, answer3, answer4, correctAnswer,id 
        },{
            headers:{
                'auth-token':token
            }
